...
if cmp -s "$TMP_BASHRC" "$LOCAL_BASHRC"; then
  echo "O .bashrc já está atualizado. Nenhuma alteração necessária."
  rm -f "$TMP_BASHRC"
  exit 0
fi

echo "Mudanças detectadas no .bashrc. Aplicando atualizações..."

# 3. Copia para o usuário atual
cp "$TMP_BASHRC" "$LOCAL_BASHRC"
echo "Atualização do .bashrc do usuário concluída."
...
echo "Atualização do .bashrc para ROOT e /etc concluída."
